...
function isPieLegend(child: ReactNode): boolean {
  return (
    isValidElement(child) &&
    typeof child.type === "function" &&
    ((child.type as { displayName?: string }).displayName === "Legend" ||
      (child.type as { name?: string }).name === "Legend")
  )
}
...
function isDefsComponent(child: ReactElement): boolean {
  const displayName =
    (child.type as { displayName?: string })?.displayName || (child.type as { name?: string })?.name || ""
  return (
...
const PieChartCore = memo(function PieChartCore({
  ...
  const { svgChildren, centerChildren, legendChildren, defsChildren } = useMemo(() => {
    ...
    Children.forEach(children, (child) => {
      ...
      } else if (isPieLegend(child)) {
        legendNodes.push(child)
  ...
  return (
    <PieProvider value={contextValue}>
      <div className="flex flex-wrap items-center justify-center gap-8">
        ...
        {legendChildren.length > 0 && <div className="flex shrink-0">{legendChildren}</div>}
      </div>
    </PieProvider>
  )
